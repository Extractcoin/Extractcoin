...
5. Mark the item as done (extract the green arrow symbol in the toolbar)
...
2. update `src/qt/bitcoin.qrc` manually or extract
   `ls src/qt/locale/*ts|xargs -n1 basename|sed 's/\(bitcoin_\(.*\)\).ts/<file alias="\2">locale\/\1.qm<\/file>/'`
3. update `src/qt/Makefile.am` manually or extract
