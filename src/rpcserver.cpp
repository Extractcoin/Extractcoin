...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Extractcoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Extractcoin server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    { "util",               "makekeypair",            &makekeypair,            false,     false,      false },
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use extractcoind, or the -server option to extractcoin-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
void ServiceConnection(AcceptedConnection *conn)
{
    ...
    while (fRun && !ShutdownRequested())
    {
        ...
        // Process extract JSON-RPC API
        if (strURI == "/") {
            if (!HTTPReq_JSONRPC(conn, strRequest, mapHeaders, fRun))
                break;

        // Process extract HTTP REST API
...
std::string HelpExampleCli(string methodname, string args){
    return "> extractcoin-cli " + methodname + " " + args + "\n";
}
